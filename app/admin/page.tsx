...
import { Package, ShoppingCart, Users, DollarSign, TrendingUp, ArrowRight, AlertTriangle } from "lucide-react";
...
export default async function AdminDashboardPage() {
  const [productCount, orderCount, userCount, totalRevenueResult, recentOrders, lowStockProducts] = await Promise.all([
    ...
    prisma.product.findMany({
      ...
      take: 5,
      select: { id: true, name: true, price: true, inventory: { select: { quantity: true } } }
    })
  ]);
  ...
  return (
    <>
      <SeoHead title="Admin Dashboard" description="Overview of store performance." />

      <div className="flex flex-col gap-8">
        {/* Page Header */}
        <div>
          <p className="text-xs uppercase tracking-widest text-primary font-semibold mb-2">Overview</p>
          <h1 className="font-display text-3xl font-bold tracking-tight">Dashboard</h1>
          <p className="text-muted-foreground mt-1 text-sm">Your store at a glance.</p>
        </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-7 gap-6">
          ...
          <div className="xl:col-span-4 relative overflow-hidden bg-card border border-border/50 rounded-xl">
            ...
            <div className="relative z-10 flex items-center justify-between px-6 py-5 border-b border-border/50">
              ...
              <Link href="/admin/orders" className="flex items-center gap-1.5 text-xs text-muted-foreground hover:text-primary transition-colors">
            ...
            <div className="relative z-10 divide-y divide-border/30">
              {recentOrders.length === 0 ? (
                <div className="px-6 py-16 text-center flex flex-col items-center justify-center">
                  ...
                  <h3 className="font-display text-lg font-bold text-foreground">No Orders Yet</h3>
                  <p className="text-sm text-muted-foreground max-w-[250px] mt-2">When customers place orders, they will appear here in real-time.</p>
                </div>
              ) : (
                recentOrders.map((order) => (
                  <div key={order.id} className="flex items-center justify-between px-6 py-4 hover:bg-white/[0.02] transition-colors group">
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-medium text-foreground truncate group-hover:text-primary transition-colors">{order.customerName}</p>
                    ...
                    <div className="flex items-center gap-6 ml-4 shrink-0">
                      ...
                      <span className="text-sm font-display font-bold tabular-nums text-foreground">{order.total.toLocaleString()} DA</span>
                    </div>
                  </div>
                ))
              )}
              ...
              <div className="flex items-center gap-2.5">
                ...
                {lowStockProducts.length > 0 && (
                  <div className="flex items-center gap-1.5 text-[10px] uppercase tracking-wider font-bold bg-orange-500/10 text-orange-400 px-2 py-0.5 rounded-full">
                    ...
                    {lowStockProducts.length} Action{lowStockProducts.length !== 1 ? 's' : ''}
                  </div>
                )}
              </div>
              <Link href="/admin/products" className="flex items-center gap-1.5 text-xs text-muted-foreground hover:text-primary transition-colors">
            ...
            <div className="relative z-10 divide-y divide-border/30">
              {lowStockProducts.length === 0 ? (
                <div className="px-6 py-16 text-center flex flex-col items-center justify-center">
                  ...
                  <h3 className="font-display text-lg font-bold text-foreground">Inventory Healthy</h3>
                  <p className="text-sm text-muted-foreground max-w-[250px] mt-2">All your products are sufficiently stocked. No action needed.</p>
                </div>
              ) : (
                lowStockProducts.map((product) => (
                  <div key={product.id} className="flex items-center justify-between px-6 py-4 hover:bg-white/[0.02] transition-colors">
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-medium text-foreground truncate">{product.name}</p>
                      <p className="text-xs text-muted-foreground mt-0.5">{product.price.toLocaleString()} DA</p>
                    </div>
                    <div className="ml-4">
                      <span className={`text-[10px] uppercase tracking-wider font-bold px-2.5 py-1 rounded-full ${
                        ...
                        {(product.inventory?.quantity || 0) === 0 ? "OUT OF STOCK" : `${product.inventory?.quantity} LEFT`}
                      </span>
                    </div>
                  </div>
                ))
