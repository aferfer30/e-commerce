...
export function AddToCartButton({ 
  ...
  const handleAddToCart = (e: React.MouseEvent) => {
    ...
    addItem({
      ...
      brand: product.brand,
      slug: product.slug,
      image: images.length > 0 ? images[0] : "https://images.unsplash.com/photo-1560393464-5c69a73c5770?auto=format&fit=crop&q=80&w=800"
    });
  ...
  return (
    ...
    >
      ...
      <div className={`${isLg ? 'w-10 h-10' : 'w-8 h-8'} rounded-full bg-black/10 flex items-center justify-center transition-all duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] group-hover:translate-x-1 group-hover:-translate-y-[1px] group-hover:scale-105`}>
        <ShoppingCart className={isLg ? "h-5 w-5" : "h-4 w-4"} strokeWidth={1.5} />
      </div>
    </button>
  );
}
